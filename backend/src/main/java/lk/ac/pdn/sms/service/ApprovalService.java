    ...
    public void processRegistrationApproval(Long id, ApprovalDto dto) {
        ...
        if (dto.getStatus() != null && dto.getStatus().contains("REJECTED")) {
            ...
            switch (reg.getStatus()) {
                case PENDING_DEAN: reg.setStatus(SocietyRegistration.ApprovalStage.PENDING_AR); break;
                case PENDING_AR: reg.setStatus(SocietyRegistration.ApprovalStage.PENDING_VC); break;
                case PENDING_VC:
    ...
    public void processRenewalApproval(Long id, ApprovalDto dto) {
        ...
        if (dto.getStatus() != null && dto.getStatus().contains("REJECTED")) {
            ...
            switch (renewal.getStatus()) {
                case PENDING_DEAN: renewal.setStatus(SocietyRenewal.RenewalStatus.PENDING_AR); break;
                case PENDING_AR: renewal.setStatus(SocietyRenewal.RenewalStatus.PENDING_VC); break;
                case PENDING_VC: renewal.setStatus(SocietyRenewal.RenewalStatus.APPROVED); break;
                default: break;
            }
        }
